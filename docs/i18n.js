...
const I18N = {
    ...
    'pt-BR': {
        ...
        'Seven Fedora images: six desktops and one telephony appliance. Pick the one that matches what you do — the tools, the drivers and the tuning are already in it, and an update that goes wrong is one command back.': 'Sete imagens Fedora: seis desktops e um appliance de telefonia. Escolha a que combina com o que você faz: as ferramentas, os drivers e o ajuste fino já vêm dentro, e uma atualização que dá errado volta com um comando.',
        'Get an image': 'Baixar uma imagem',
        'Build it yourself': 'Construa você mesmo',
        'Editions': 'Edições',
        'Based on Fedora 44': 'Baseado no Fedora 44',
        'plus one headless appliance': 'mais um appliance sem desktop',
        ...
        // Accessibility labels
        'Section': 'Seção',
        'Layer chain: quay.io/fedora/fedora-bootc:44, then base, which forks into desktop-common and de/cosmic-hyprland carrying the six desktop purposes, and de/headless carrying the pbx appliance': 'Cadeia de camadas: quay.io/fedora/fedora-bootc:44, depois base, que se bifurca em desktop-common e de/cosmic-hyprland com os seis propósitos de desktop, e em de/headless com o appliance pbx',
    },
    ja: {
        'Late shift.': '夜勤お疲れさま。',
        ...
        'Seven Fedora images: six desktops and one telephony appliance. Pick the one that matches what you do — the tools, the drivers and the tuning are already in it, and an update that goes wrong is one command back.': 'Fedoraイメージは7種類: デスクトップ6つと電話交換機アプライアンス1つ。用途に合うものを選ぶだけ — ツール、ドライバー、チューニングは組み込み済み。更新に失敗してもコマンド1つで元に戻せます。',
        'Get an image': 'イメージを入手',
        'Build it yourself': '自分でビルド',
        'Editions': 'エディション',
        'Based on Fedora 44': 'Fedora 44ベース',
        'plus one headless appliance': '＋デスクトップなしのアプライアンス1つ',
        ...
        // Accessibility labels
        'Section': 'セクション',
        'Layer chain: quay.io/fedora/fedora-bootc:44, then base, which forks into desktop-common and de/cosmic-hyprland carrying the six desktop purposes, and de/headless carrying the pbx appliance': 'レイヤーチェーン: quay.io/fedora/fedora-bootc:44、次に base。そこから desktop-common と de/cosmic-hyprland（6つのデスクトップ用途）と、de/headless（pbxアプライアンス）に分岐します',
    },
    ru: {
        ...
        'Seven Fedora images: six desktops and one telephony appliance. Pick the one that matches what you do — the tools, the drivers and the tuning are already in it, and an update that goes wrong is one command back.': 'Семь образов Fedora: шесть рабочих столов и один телефонный сервер. Выберите тот, что соответствует вашим задачам: инструменты, драйверы и тюнинг уже внутри, а неудачное обновление откатывается одной командой.',
        'Get an image': 'Получить образ',
        'Build it yourself': 'Соберите сами',
        'Editions': 'Редакции',
        'Based on Fedora 44': 'На основе Fedora 44',
        'plus one headless appliance': 'плюс один сервер без рабочего стола',
        ...
        'its parent, so a shared layer is built once and reused by everything below it. The appliance branch skips the desktop entirely. The gold leaves are the published images; intermediates are build infrastructure.': 'от родительского, поэтому общий слой собирается один раз и переиспользуется всем, что лежит ниже него. Ветка телефонного сервера полностью обходит рабочий стол. Золотые листья — публикуемые образы; промежуточные слои — инфраструктура сборки.',
        ...
        // Accessibility labels
        'Section': 'Раздел',
        'Layer chain: quay.io/fedora/fedora-bootc:44, then base, which forks into desktop-common and de/cosmic-hyprland carrying the six desktop purposes, and de/headless carrying the pbx appliance': 'Цепочка слоёв: quay.io/fedora/fedora-bootc:44, затем base, который разветвляется на desktop-common с de/cosmic-hyprland (шесть назначений с рабочим столом) и на de/headless с образом pbx',
    },
};
...
function applyAttrI18n() {
    ...
    if (chain) chain.setAttribute('aria-label', t('Layer chain: quay.io/fedora/fedora-bootc:44, then base, which forks into desktop-common and de/cosmic-hyprland carrying the six desktop purposes, and de/headless carrying the pbx appliance'));
}
